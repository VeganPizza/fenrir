...
void ax12writePose(int *pose, int *id, int size){
    int length = 4 + (size * 3);   // 3 = id + pos(2byte)
    ...
    {
        checksum += (pose[i]&0xff) + (pose[i]>>8) + id[i];
        ax12write(id[i]);
        ax12write(pose[i]&0xff);
        ax12write(pose[i]>>8);
    } 
...
void ax12writePose2(int *pose, int *id, int size, int speed){
    int length = 4 + (size * 5);   // 5 = id + pos(2byte)+speed(2bytes)
    ...
    for(int i=0; i<size; i++)
    {
        checksum += (pose[i]&0xff) + (pose[i]>>8) +(speed&0xff)+(speed>>8)+ id[i];
        ax12write(id[i]);
        ax12write(pose[i]&0xff);
        ax12write(pose[i]>>8);
